...
import fr.smo.chess.core.GameState.GameOutcome.*
...
class MoveApplierTest {
    ...
    inner class CheckMate {
        ...
        fun `should mark game as won when performing Scholar's mate`() {
            // Given
            var gameState = GameVariants.createStandardGame()
            gameState = moveApplier.applyMoveRequest(MoveRequest(E2,E4), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(E7,E5), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(F1,C4), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(B8,C6), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(D1,H5), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(G8,F6), gameState)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H5,F7), gameState)
            // Then
        ...
        fun `should mark game as won when performing back rank mate`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H8,H1), gameState)
            // Then
        ...
        fun `should mark game as won when performing kiss of death checkmate`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E2,B2), gameState)
            // Then
            expectThat(gameState.gameOutcome) isEqualTo BLACK_WIN
        }
        ...
        fun `should mark game as won when performing smothered mate checkmate`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(D6,F7), gameState)
            // Then
            expectThat(gameState.gameOutcome) isEqualTo WHITE_WIN
        }
        ...
        fun `should mark game as won when performing two rook checkmate`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(D7,H7), gameState)
            // Then
            expectThat(gameState.gameOutcome) isEqualTo WHITE_WIN
        }
        ...
        fun `should mark game as won when performing rook + king checkmate`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H2,H1), gameState)
            // Then
            expectThat(gameState.gameOutcome) isEqualTo BLACK_WIN
        }
        ...
        fun `should mark game as won after a promotion checkmate`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H2,H1, QUEEN), gameState)
            // Then
            expectThat(gameState.gameOutcome) isEqualTo BLACK_WIN
        }
    }
    ...
    inner class CasualMove {
        ...
        fun `should pawn moved without capture lead to no pieces amount changed on chessboard`() {
            ...
            var gameState = GameVariants.createStandardGame()
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H2,H4), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(H7, H5 ),gameState)
            // Then
    ...
    inner class CheckSituation {
        ...
        fun `should throw an error when performing a move that doesn't revert check when king is checked`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H2,H1), gameState) // White King check

            // Then
            assertThrows<IllegalStateException> {
                moveApplier.applyMoveRequest(MoveRequest(B7,B8), gameState)
            }
        }
        ...
        fun `should throw an error when performing a move that doesn't revert check when king is checked after promotion`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H2,H1, QUEEN), gameState) // White King check
    ...
    inner class Promotion {
        ...
        fun `should black pawn promote to a Queen when possible`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(B2,B1, QUEEN), gameState)
            // Then
        ...
        fun `should black pawn promote to a Knight when possible`() {
            ...
            var gameState = givenAChessGame(fenPiecePlacementOnly = "4k3/P7/8/8/8/8/1p6/4K3", sideToMove = BLACK)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(B2,B1, KNIGHT), gameState)
            // Then
        ...
        fun `should black pawn promote to a Bishop when possible`() {
            ...
            var gameState = givenAChessGame(fenPiecePlacementOnly = "4k3/P7/8/8/8/8/1p6/4K3", sideToMove = BLACK)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(B2,B1, BISHOP), gameState)
            // Then
        ...
        fun `should black pawn promote to a Rook when possible`() {
            ...
            var gameState = givenAChessGame(fenPiecePlacementOnly = "4k3/P7/8/8/8/8/1p6/4K3", sideToMove = BLACK)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(B2,B1, ROOK), gameState)
            // Then
        ...
        fun `should white pawn promote to a Queen when possible`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A7,A8, QUEEN), gameState)
            // Then
        ...
        fun `should white pawn promote to a Knight when possible`() {
            ...
            var gameState = givenAChessGame(fenPiecePlacementOnly = "4k3/P7/8/8/8/8/1p6/4K3", sideToMove = WHITE)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A7,A8, KNIGHT), gameState)
            // Then
        ...
        fun `should white pawn promote to a Bishop when possible`() {
            ...
            var gameState = givenAChessGame(fenPiecePlacementOnly = "4k3/P7/8/8/8/8/1p6/4K3", sideToMove = WHITE)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A7,A8, BISHOP), gameState)
            // Then
        ...
        fun `should white pawn promote to a Rook when possible`() {
            ...
            var gameState = givenAChessGame(fenPiecePlacementOnly = "4k3/P7/8/8/8/8/1p6/4K3", sideToMove = WHITE)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A7,A8, ROOK), gameState)
            // Then
    ...
    inner class CapturedPiece {
        ...
        fun `should remove piece from chessboard after capture`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(D4,E5), gameState)
            // Then
    ...
    inner class Draw {
        ...
        fun `should mark game outcome as draw regarding the 50 moves rule`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A8,A1), gameState)
            // Then
        ...
        fun `should mark game outcome as draw when game history has a threefold repetition situation`() {
            ...
            gameState = moveApplier.applyMoveRequest(MoveRequest(H1,G1), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(G8,H8), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(G1,H1), gameState)
            gameState = moveApplier.applyMoveRequest(MoveRequest(H8,G8), gameState)
        ...
        inner class Stalemate {
            ...
            fun `should mark game outcome as draw with a simple stalemate`() {
                ...
                // when
                gameState = moveApplier.applyMoveRequest(MoveRequest(E7,F7), gameState)
                // Then
                expectThat(gameState.gameOutcome) isEqualTo DRAW
            }
            ...
            fun `should mark game outcome as draw with a simple stalemate part 2`() {
                ...
                // when
                gameState = moveApplier.applyMoveRequest(MoveRequest(A4,A3), gameState)
                // Then
                expectThat(gameState.gameOutcome) isEqualTo DRAW
            }
            ...
            fun `should mark game outcome as draw with a more complex stalemate (pinned king)`() {
                ...
                // when
                gameState = moveApplier.applyMoveRequest(MoveRequest(E1,F3), gameState)
                // Then
                expectThat(gameState.gameOutcome) isEqualTo DRAW
            }
        }
    ...
    inner class HalfClockMove {
        ...
        fun `should test increment of half clock move in game state after not a pawn move`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A1,A2), gameState)
            // Then
        ...
        fun `should test rest half clock move in game state after pawn move`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(D3,D4), gameState)
            // Then
        ...
        fun `should test rest half clock move in game state after captured piece`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H8,H6), gameState)
            // Then
            expectThat(gameState.halfMoveClock) isEqualTo 0
        }
    }
    ...
    inner class EnPassant {
        ...
        fun `should change en passant target square in game state and set it back to null when necessary`() {
            ...
            gameState = moveApplier.applyMoveRequest(MoveRequest(D2, D3), gameState)
        ...
        fun `should capture en passant to the right for white`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E5,F6), gameState)
            // Then
            expectThat(gameState.enPassantTargetSquare).isNull()
        ...
        fun `should capture en passant to the left for white`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H5,G6), gameState)
            // Then
            expectThat(gameState.enPassantTargetSquare).isNull()
        ...
        fun `should capture en passant to the right for black`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(C4,D3), gameState)
            // Then
            expectThat(gameState.enPassantTargetSquare).isNull()
        ...
        fun `should capture en passant to the left for black`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(F4,E3), gameState)
            // Then
            expectThat(gameState.enPassantTargetSquare).isNull()
    ...
    inner class Castle {
        ...
        fun `should lose ability to black queen castle if a8 rook moves`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(A8,A7), gameState)
            // Then
        ...
        fun `should lose ability to black queen castle after a8 black rook captured`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(D5,A8), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isFalse()
            expectThat(gameState.isBlackKingCastlePossible).isTrue()
        ...
        fun `should lose ability to black king castle after h8 black rook captured`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E5,H8), gameState)
            // Then
        ...
        fun `should lose ability to white queen castle after a1 black rook captured`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(D4,A1), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isFalse()
            expectThat(gameState.isBlackKingCastlePossible).isFalse()
            expectThat(gameState.isWhiteKingCastlePossible).isTrue()
        ...
        fun `should lose ability to white king castle after h1 black rook captured`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E4,H1), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isFalse()
            expectThat(gameState.isBlackKingCastlePossible).isFalse()
            expectThat(gameState.isWhiteKingCastlePossible).isFalse()
        ...
        fun `should lose ability to black king castle if h8 rook moves`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H8,H7), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isTrue()
            expectThat(gameState.isBlackKingCastlePossible).isFalse()
            expectThat(gameState.isWhiteKingCastlePossible).isTrue()
        ...
        fun `should lose ability to white king castle if h1 rook moves`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(H1,H2), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isTrue()
            expectThat(gameState.isBlackKingCastlePossible).isTrue()
            expectThat(gameState.isWhiteKingCastlePossible).isFalse()
        ...
        fun `should loose all abilities to white castle moves wen white king moves`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E1,E2), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isTrue()
            expectThat(gameState.isBlackKingCastlePossible).isTrue()
            expectThat(gameState.isWhiteKingCastlePossible).isFalse()
        ...
        fun `should loose all black castle moves wen black king moves`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E8,E7), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isFalse()
            expectThat(gameState.isBlackKingCastlePossible).isFalse()
            expectThat(gameState.isWhiteKingCastlePossible).isTrue()
        ...
        fun `should perform white king castle move when possible`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E1,G1), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isTrue()
            expectThat(gameState.isBlackKingCastlePossible).isTrue()
            expectThat(gameState.isWhiteKingCastlePossible).isFalse()
        ...
        fun `should perform white queen castle move when possible`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E1,C1), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isTrue()
            expectThat(gameState.isBlackKingCastlePossible).isTrue()
            expectThat(gameState.isWhiteKingCastlePossible).isFalse()
        ...
        fun `should perform black king castle move when possible`() {
            ...
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E8,G8), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isFalse()
            expectThat(gameState.isBlackKingCastlePossible).isFalse()
            expectThat(gameState.isWhiteKingCastlePossible).isTrue()
        ...
        fun `should perform black queen castle move when possible`() {
            ...
            var gameState = givenAChessGame(
                ...
                sideToMove = BLACK)
            // When
            gameState = moveApplier.applyMoveRequest(MoveRequest(E8,C8), gameState)
            // Then
            expectThat(gameState.isBlackQueenCastlePossible).isFalse()
            expectThat(gameState.isBlackKingCastlePossible).isFalse()
            expectThat(gameState.isWhiteKingCastlePossible).isTrue()
        ...
        fun `should throw an error when attempting to castle and king moving squares are under attack`() {
            ...
            // When
            assertThrows<IllegalStateException> {
                moveApplier.applyMoveRequest(MoveRequest(E8,C8), gameState)
            }
        }
